      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 bg-green-500/10 border border-green-500/20 rounded-full px-3 py-1 text-green-400 text-sm font-medium">
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <div className="flex space-x-1 bg-slate-800/50 backdrop-blur-xl rounded-2xl p-1">
            {[
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50">
                ...
                <div className="text-3xl font-bold text-white">{profile?.totalTrades || 0}</div>
                <div className="text-green-400 text-sm">+{recentTrades.length} this month</div>
              </div>
              ...
              <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50">
                ...
                <div className="text-3xl font-bold text-white">{profile?.trustScore || 0}</div>
                <div className="text-blue-400 text-sm">Based on {profile?.totalRatings || 0} ratings</div>
              </div>
              ...
              <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50">
                ...
                <div className="text-3xl font-bold text-white">{formatAmount(profile?.totalTrades ? profile.totalTrades * 1000 : 0)}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h3 className="text-xl font-semibold text-white mb-4">Your Trust Score</h3>
                {profile && (
                  <TrustScoreCard
                    profile={profile}
                  />
              ...
              <div>
                ...
                <div className="space-y-3">
                  <motion.button
                    ...
                    className="w-full p-4 bg-gradient-to-r from-blue-500 to-purple-500 rounded-xl text-white font-medium hover:from-blue-600 hover:to-purple-600 transition-colors"
                  >
                    Start New Trade
                  </motion.button>
                  <motion.button
                    ...
                    className="w-full p-4 bg-slate-700/50 hover:bg-slate-700 border border-slate-600 rounded-xl text-white font-medium transition-colors"
                  >
                    Find Traders
                  </motion.button>
                </div>
                ...
                <div className="space-y-3">
                  {recentTrades.length > 0 ? (
                    recentTrades.map((trade) => (
                      <div key={trade.id} className="p-4 bg-slate-800/50 backdrop-blur-xl rounded-xl border border-slate-700/50">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span>{getTradeStatusIcon(trade.status)}</span>
                            <span className="text-white font-medium">Trade #{trade.id.slice(0, 8)}</span>
                          </div>
                          <span className={`text-sm font-medium ${getTradeStatusColor(trade.status)}`}>
                            {trade.status}
                          </span>
                        </div>
                ...
                <div className="space-y-3">
                  {recentRatings.length > 0 ? (
                    recentRatings.map((rating) => (
                      <div key={rating.id} className="p-4 bg-slate-800/50 backdrop-blur-xl rounded-xl border border-slate-700/50">
                        <div className="flex items-center justify-between mb-2">
